...
- Name: TM UI 补充 rmtauw2id >> C Lead 发布 Action；复制到本周（同周 clone）
- Location: D:\代码\testai_community\frontend\e2e\tm-ui-extra.spec.ts:85:3
...
Locator: locator('.ant-select-dropdown:not(.ant-select-dropdown-hidden)').last().locator('.ant-select-item-option-content').filter({ hasText: '【E2E】rmtauw2id S2' }).first()
Expected: visible
Received: <element(s) not found>
Call log:
  - expect.toBeVisible with timeout 20000ms
  - waiting for locator('.ant-select-dropdown:not(.ant-select-dropdown-hidden)').last().locator('.ant-select-item-option-content').filter({ hasText: '【E2E】rmtauw2id S2' }).first()

    at antdSelectByLabel (D:\代码\testai_community\frontend\e2e\helpers.ts:107:32)
    at D:\代码\testai_community\frontend\e2e\tm-ui-extra.spec.ts:94:5
```
...
- banner:
  ...
  - button "moon":
    - img "moon"
  - button "user e2eLb_rmtauw2id":
    - img "user"
    - text: e2eLb_rmtauw2id
- main:
  ...
  - tabpanel "工作台":
    ...
    - text: 全部（1）
    - combobox
    - text: 【E2E】rmtauw2id P2 【E2E】rmtauw2id T2 进行中 【E2E】rmtauw2id P2/【E2E】rmtauw2id D2 本周无 Action 0%
    - img "exclamation-circle"
    - button "操作 down":
      - text: 操作
      - img "down"
    - button "+ Action"
    - text: 测试中 测试开始 08-01 ~ 预计结束 08-01 · 需求：补测 · 负责人 LeadBrmtauw2id
    - img "No data"
    ...
    - list:
      ...
      - listitem:
        - combobox "Page Size"
        - text: 10 / page
- dialog "新建 Action · 【E2E】rmtauw2id T2":
  - button "Close":
    - img "close"
  - text: 新建 Action · 【E2E】rmtauw2id T2 * 关联子需求
  - combobox "* 关联子需求"
  - text: 选择子需求 * 标题
  - textbox "* 标题"
  - text: 0 / 300 * 本周负责人
  - combobox "* 本周负责人"
  - text: LeadBrmtauw2id（e2eLb_rmtauw2id） 测试内容
  - textbox "测试内容"
  - text: 0 / 1000 环境
  - textbox "环境"
  - text: 0 / 300
