...
def seed_action_lineage_demo(
    *,
    ...
    from app.test_manage.schemas import ActionCloneRequest, ActionCreate, TaskCreate
    ...
    try:
        ...
        if not proj:
            from app.test_manage.schemas import ProjectCreate
        ...
        if not dom:
            dout = create_domain(db, admin, proj.id, DEMO_DOMAIN_NAME)
        ...
        # 挪到「上一业务周」，才能：① 出现在复制上周；② 切周日仍可写日更（日更归属刚结束周）
