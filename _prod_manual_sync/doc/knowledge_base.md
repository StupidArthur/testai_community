...
知识库是 TestAI Community 的 **RAG（检索增强生成）** 模块。现网为 **全站共用一个知识库**：文档先经 **清洗入库**（切分、挂锚点、人工审核），批准后再参与问答。

**核心特点：**

- **单库 Hub**：顶栏「知识库」进入 `/knowledge-base`（知识问答 + 清洗入库两个 Tab）
- **清洗后入库**：上传 → 自动切分 → 审核页确认 → **批准入库** 后可提问
...
审核页只需关注：**精华内容是否正确**、**入库操作**（新增 / 替换 / 跳过等）。  
系统在后台会自动打主题标签（不对用户展示），用于内部对齐；**无需维护锚点词典、无需手选锚点**。
