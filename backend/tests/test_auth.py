...
class TestUserManagement:
    ...
    def test_list_users_eng_forbidden(self, client, eng_token):
        r = client.get("/api/users", headers={"Authorization": f"Bearer {eng_token}"})
        assert r.status_code == 403

    def test_get_me(self, client, admin_token):
        r = client.get("/api/users/me", headers={"Authorization": f"Bearer {admin_token}"})
        assert r.status_code == 200
...
class TestTicketMechanism:
    ...
    def test_ticket_single_use(self, client, admin_token):
        ...
        r1 = client.get(f"/api/translate/jobs?ticket={ticket}")
        assert r1.status_code == 200

        r2 = client.get(f"/api/translate/jobs?ticket={ticket}")
        assert r2.status_code == 401

    def test_invalid_ticket(self, client):
        r = client.get("/api/translate/jobs?ticket=fake_ticket_12345")
        assert r.status_code == 401
